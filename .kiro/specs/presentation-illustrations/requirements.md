# Documento de Requisitos: Ilustraciones para Presentación Reveal.js "De 0 a Observabilidad"
...
7. THE VM_Container_Diagram SHALL mantener el atributo `data-auto-animate` y `data-transition="convex"` en el `<section>` padre, preservando el comportamiento de transición original.
8. IF el recurso SVG externo de un ícono no carga desde la CDN, THEN THE VM_Container_Diagram SHALL seguir siendo legible mediante los labels de texto inline SVG y las formas geométricas.
...
1. WHEN el presentador navega al slide "Arquitectura", THE Presentation SHALL mostrar el `Control_Plane_Diagram` en lugar del `.grid-2` con 7 cards `.fragment` existentes.
...
6. WHEN el presentador avanza al fragment `data-fragment-index="5"`, THE Control_Plane_Diagram SHALL revelar simultáneamente las flechas bidireccionales entre Control Plane y Worker Node, y el componente kubelet con texto "Agente del nodo · gestiona pods".
...
3. WHEN el presentador avanza el fragment con `data-fragment-index="1"`, THE URL_Flow_Diagram SHALL revelar el componente Browser con una representación simplificada de ventana de navegador y el texto "GET /abc123".
4. WHEN el presentador avanza al fragment `data-fragment-index="2"`, THE URL_Flow_Diagram SHALL revelar simultáneamente la flecha "HTTP" desde Browser hacia Frontend y el componente Frontend con el logo React (elipses rotadas) y textos "Frontend", "React · Vite · Nginx".
...
6. WHEN el presentador avanza al fragment `data-fragment-index="4"`, THE URL_Flow_Diagram SHALL revelar simultáneamente la flecha "GET key" desde Backend hacia Redis y el componente Redis con el texto "url:{code} → URL".
7. WHEN el presentador avanza al fragment `data-fragment-index="5"`, THE URL_Flow_Diagram SHALL revelar un arco de retorno curvo de color `#2ecc71` con línea discontinua desde Redis de vuelta hacia Browser, con la etiqueta "301 Redirect → URL original".
8. WHEN el presentador avanza al fragment `data-fragment-index="6"`, THE URL_Flow_Diagram SHALL revelar un rectángulo perimetral discontinuo con la etiqueta "Docker network: shortener_default".
...
10. THE Observability_Stack_Diagram SHALL preservar el subtítulo existente "Los 3 pilares → un solo panel de control" bajo el `<h3>`.
...
5. WHEN el presentador avanza al fragment `data-fragment-index="1"`, THE Roadmap_Pipeline SHALL revelar el nodo 1 con ícono de Docker cargado desde CDN y el texto "Contenedores / Docker · Imágenes".
...
6. IF un ícono SVG externo no carga desde CDN, THEN THE Presentation SHALL mantener legibilidad completa mediante los labels de texto y formas geométricas inline SVG de cada ilustración.
